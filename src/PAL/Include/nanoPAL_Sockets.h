...
#define SOCK_MAKE_IP_ADDR(w,x,y,z)           ((uint32_t)(w) & 0xFF)<<24 | ((uint32_t)(x) & 0xff)<<16 | ((uint32_t)(y) & 0xff)<<8 | ((uint32_t)(z) & 0xff)
#define SOCK_MAKE_IP_ADDR_LITTLEEND(w,x,y,z) ((uint32_t)(z) & 0xFF)<<24 | ((uint32_t)(y) & 0xff)<<16 | ((uint32_t)(x) & 0xff)<<8 | ((uint32_t)(w) & 0xff)
...
bool  SOCKETS_UpgradeToSsl( int ComPortNum, const uint8_t* pCACert, uint32_t caCertLen, const uint8_t* pDeviceCert, uint32_t deviceCertLen, LPCSTR szTargetHost );
