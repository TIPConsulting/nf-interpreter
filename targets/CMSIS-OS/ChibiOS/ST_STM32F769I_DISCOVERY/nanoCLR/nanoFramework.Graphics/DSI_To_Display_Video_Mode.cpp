...
void DisplayInterface::Initialize(DisplayInterfaceConfig &config)
{
    ...
    __NVIC_SetPriority(
        ...
        NVIC_EncodePriority(
            ...
            3,
            0)); // NVIC configuration for LTDC interrupt that is now enabled
    ...
    __NVIC_SetPriority(
        ...
        NVIC_EncodePriority(
            NVIC_GetPriorityGrouping(),
            3,
            0)); // NVIC configuration for DSI interrupt that is now enabled
    ...
    DsiRegister->WPCR[0U] |=
        (4000000U * tempIDF * ((1UL << (0x3U & PLLODF)))) / ((((uint32_t)25000000) / 1000U) * PLLNDIV);
    ...
    DsiRegister->LPCR |=
        (0x00000000U | 0x00000000U | 0x00000000U); // Vertical, Horizontal active high, DSI data enable active high
    ...
    DsiRegister->VLCR |= ((HACT + HSA + HBP + HFP) * laneByteClk_kHz) /
    ...
    LTDC_Layer_Register->WHPCR =
        ((((LtdcRegister->BPCR & LTDC_BPCR_AHBP) >> 16U) + 1U) |
         ((lcd_x_size + ((LtdcRegister->BPCR & LTDC_BPCR_AHBP) >> 16U)) << 16U));

    // Configure the vertical start and stop position
    LTDC_Layer_Register->WVPCR &= ~LTDC_LxWVPCR_WVSTPOS | LTDC_LxWVPCR_WVSPPOS;
    LTDC_Layer_Register->WVPCR =
        (((LtdcRegister->BPCR & LTDC_BPCR_AVBP) + 1U) | ((lcd_y_size + (LtdcRegister->BPCR & LTDC_BPCR_AVBP)) << 16U));
...
void DisplayInterface::SendCommand(CLR_UINT8 NbrParams, ...)
{
    ...
    switch (NbrParams)
    {
        ...
        case 2:
        {
            ...
            DsiRegister->GHCR =
                (DSI_DCS_SHORT_PKT_WRITE_P1 | (LCD_OTM8009A_ID << 6U) | (command << 8U) | (parameter << 16U));
            break;
        }
        default:
        {
            ...
            DsiRegister->GHCR =
                (DSI_DCS_LONG_PKT_WRITE | (LCD_OTM8009A_ID << 6U) | (((NbrParams + 1U) & 0x00FFU) << 8U) |
                 ((((NbrParams + 1U) & 0xFF00U) >> 8U) << 16U)); // Update the DSI packet header with new information
        }
    }
}
void WaitForFIFOEmpty()
